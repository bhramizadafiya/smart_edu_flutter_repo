  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        title: const Text(
          "Configure Your Test",
          style: TextStyle(fontSize: 18,fontWeight: FontWeight.w800, color: Color.fromARGB(255, 14, 49, 34)),
        ),
        centerTitle: true,
            ...
            child: Column(
              ...
              children: [
                ...
                Container(
                  ...
                  child: Column(
                    children: [
                      const Icon(Icons.settings_outlined, color: Color.fromARGB(255, 14, 49, 34), size: 42),
                      const SizedBox(height: 16),
                      const Text(
                        "Customize Your Test",
                        style: TextStyle(fontSize: 22, fontWeight: FontWeight.w900, color: Color.fromARGB(255, 14, 49, 34)),
...
const Padding(
  ...
  child: Text("Question Types", style: TextStyle(fontSize: 18, fontWeight: FontWeight.w900, color: Color.fromARGB(255, 14, 49, 34))),
),
...
Padding(
  ...
  child: Column(
    children: [
      // Multiple Choice → Blue checkbox
      // In your view, use normal variables like before:
_buildQuestionTypeCard(
  title: "Multiple Choice Questions",
  subtitle: "Single correct answer from 4 options",
  value: controller.multipleChoice,  // ← Use this
...
Column(
  ...
  children: [
    ...
    // Light blue area — touches top & bottom grey lines perfectly
    Container(
      color: const Color(0xFFF8FBFF), // Soft light blue
      padding: const EdgeInsets.fromLTRB(16, 16, 16, 20), // left-aligned + perfect spacing
      child: Column(
        ...
        children: [
          ...
          // Your duration cards (white background, own borders)
          Obx(() => Column(
            children: controller.durationOptions.map((option) {
              ...
              final bool isSelected = controller.selectedDuration.value == key;
    ...
    // Bottom grey line — touches light blue perfectly
    Container(
      width: double.infinity,
      height: 1,
      color: Colors.grey.shade300,
    ...
    const SizedBox(height: 16),
                ...
                const Padding(
                  padding: EdgeInsets.symmetric(horizontal: 16),
                  child: Text("Difficulty Level", style: TextStyle(fontSize: 18, fontWeight: FontWeight.w900, color: Color.fromARGB(255, 14, 49, 34))),
                ),
                const SizedBox(height: 16),
                Padding(
                  ...
                  child: Obx(() => Row(
                        children: [
                          _difficultyChip(
                            ...
                            bgColor: Color.fromARGB(255, 238, 255, 248),
                            borderColor: Color.fromARGB(255, 36, 203, 131),
  ...
  children: [
            ...
            Row(
              children: [
                const Icon(Icons.auto_awesome,
                    color: Color.fromARGB(255, 36, 203, 131), size: 30),
    ...
    // ========== DARK GREEN BAR AT THE END ==========
    Container(
      width: double.infinity,
      height: 1.5,
      color: const Color.fromARGB(255, 32, 188, 110), // Beautiful dark green
    ),
  ],
)),
...
Widget _buildQuestionTypeCard({
  ...
  required Color checkboxColor,  // Blue, Green, Red for each item
}) {
  return Obx(() => Card(
        elevation: 0,
        ...
        shape: RoundedRectangleBorder(
          ...
          side: BorderSide(
            color: Colors.grey.shade300,
            width: value.value ? 1.8 : 1.8, // Thicker when selected
          ),
        ),
        ...
        child: InkWell(
          ...
          child: Padding(
            ...
            child: Row(
              children: [
                // Checkbox on LEFT — Colored (Blue/Green/Red)
                Container(
                  width: 20,
                  height: 20,
                  decoration: BoxDecoration(
                    ...
                    border: Border.all(
                      color: value.value ? checkboxColor : Colors.grey.shade400,
                      width: 1.8,
                    ),
                  ),
                  child: value.value
                ...
                Expanded(
                  child: Column(
                    crossAxisAlignment: CrossAxisAlignment.start,
                    children: [
                      // Title: Light grey → Dark Green when selected
                      Text(
                        title,
                        style: TextStyle(
                          fontSize: 16,
                          fontWeight: FontWeight.w600,
                          color: value.value
                              ? Color.fromARGB(255, 14, 49, 34)   // Dark Green when selected
                              : Colors.grey.shade600,   // Light grey when unselected
                        ),
                      ),
                      const SizedBox(height: 4),
                      // Subtitle always grey
                      Text(
                        subtitle,
 ...
 Widget _buildDurationCard({
  ...
  // Subtle light green when selected, pure white when not
  final Color cardBgColor = isSelected
      ? const Color.fromARGB(255, 245, 255, 245)   // Very soft light green
      : Colors.white;

  // Text colors
  final Color primaryTextColor = isSelected
      ? Colors.black87           // Deep black when selected
      : Colors.grey.shade600;    // Light grey when not selected

  final Color subtitleColor = isSelected
      ? const Color.fromARGB(255, 78, 175, 255)  // Light blue
      ...
      child: Padding(
        ...
        child: Row(
          ...
          children: [
                    ...
                    Row(
                      children: [
                          ...
                          child: Container(
                            ...
                            child: DropdownButtonHideUnderline(
                              child: DropdownButton<int>(
                                ...
                                selectedItemBuilder: (_) => controller.hoursList.map((h) => Center(
                                  child: Text(
                                ...
                                items: controller.hoursList.map((h) => DropdownMenuItem(
                                  value: h,
                                  child: Center(child: Text(h.toString().padLeft(2, '0'))),
                                )).toList(),
                                onChanged: isSelected ? (v) => controller.customHours.value = v! : null,
                              ),
                            ),
                          ),
                        ),
                        ...
                        const Padding(
                          ...
                          child: Text("hours", style: TextStyle(fontSize: 11.5, color: Colors.grey)),
                        ),
                          ...
                          child: Container(
                            ...
                            child: DropdownButtonHideUnderline(
                              child: DropdownButton<int>(
                                ...
                                icon: const SizedBox.shrink(),
                                alignment: Alignment.center,
                                selectedItemBuilder: (_) => controller.minutesList.map((m) => Center(
                                  child: Text(
                                ...
                                items: controller.minutesList.map((m) => DropdownMenuItem(
                                  value: m,
                                  child: Center(child: Text(m.toString().padLeft(2, '0'))),
                                )).toList(),
                                onChanged: isSelected ? (v) => controller.customMinutes.value = v! : null,
                              ),
                            ),
                          ),
                        ),
                        ...
                        const Padding(
                          ...
                          child: Text("minutes", style: TextStyle(fontSize: 11.5, color: Colors.grey)),
                        ),
                      ],
                    ),
                ],
            ...
            // SUBTITLE – Light blue when selected
      ...
      child: GestureDetector(
        ...
        child: Container(
          ...
          decoration: BoxDecoration(
            ...
            borderRadius: BorderRadius.circular(8),
            border: Border.all(color: isSelected ? borderColor : Colors.grey.shade300, width: isSelected ? 1.9 : 1.5),
          ),
          child: Row(
            mainAxisAlignment: MainAxisAlignment.center,
            children: [
              Icon(icon, color: isSelected ? textColor : Colors.grey.shade600, size: 22),
              const SizedBox(width: 8),
              Text(label, style: TextStyle(fontSize: 15, fontWeight: FontWeight.bold, color: isSelected ? textColor : Colors.black87)),
            ],
          ),
        ),
      ),
